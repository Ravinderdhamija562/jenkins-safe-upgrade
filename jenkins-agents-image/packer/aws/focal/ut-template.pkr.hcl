...
locals {
  ...
  instance_name = "company-jenkins-ubuntu-packer-ub2004-${local.timestamp}"
}
...
source "amazon-ebs" "ubuntu_ut_ami" {
  ...
  source_ami_filter {
    filters = {
      name = "*company-jenkins-ubuntu-20.04-2025*"
    }
    most_recent = true
    owners      = ["156041411903"]
  }
  ...
  ami_description = "Ubuntu ${var.ubuntu_version} LTS Builder Image on ${local.iso_time} with base image ub20 base ami"
  vpc_id          = var.vpc_id
  ...
  tags = {
    ...
    Name                      = "company-jenkins-ubuntu-focal-20.04-ut-${local.timestamp}"
  }
}
